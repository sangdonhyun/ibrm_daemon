...
class test():
    ...
    def get_starting_job(self,status):
        ...
        sql = """
            ...
            FROM 
              ...
              where
               tjm.job_exec_dt = to_char(now(), '{YYYYMMDD}')  -- 실행일자로 변경하여 사용   

            and tjd.tg_job_dtl_id in (select tg_job_dtl_id from store.hs_job_dtl where job_exec_dt = '{YYYYMMDD}' and job_stat = '{STATUS}')

            order by mj.exec_time   desc
    ...
    def starting_job(self):
        ...
        for job in job_list:
            # print job
            print cnt, job['shell_name'], job['job_exec_dt'], job['job_exec_time'],job['job_id'],job['tg_job_dtl_id']
            cnt = cnt + 1
        print '-'*50
